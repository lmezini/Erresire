...
def get_galaxy_params_for_kwargs(galaxy_function, lens_catalog_object):
    ...
    lens_catalog_object : pandas.DataFrame
    ...
    func_args : dict
        ...
        ``lens_catalog_object``. Only parameters present in the catalog are included.

    Notes
    -----
    - The function assumes ``lens_catalog_object`` has exactly one row.
    ...
    for var in varnames:
        if var in lens_catalog_object.columns:
            func_args[var] = lens_catalog_object[var].values[0]
        else:
            warnings.warn(
...
def get_params_for_any_lenstronomy_profile(profile, catalog_object):
    """
    Generate a parameter dictionary for a given Lenstronomy profile from a catalog entry.

    This function extracts the required parameters for any Lenstronomy profile class
    and matches them to columns in a provided catalog object. It raises a warning
    for any parameters required by the profile that are missing in the catalog.

    Parameters
    ----------
    profile : class
        A Lenstronomy profile class (not a string). For example:
        ``lenstronomy.LensModel.Profiles.cnfw.CNFW``.
        The class must have the attribute ``param_names``, which lists the
        required parameters for initialization.
    catalog_object : pandas.DataFrame
        A single-row DataFrame representing the object (galaxy, halo, etc.) whose
        properties are used to fill the profile parameters. Column names must
        match the lowercased names of the profile's ``param_names``.

    Returns
    -------
    func_args : dict
        Dictionary mapping parameter names (lowercased) to values extracted
        from ``catalog_object``. Only parameters present in the catalog are included.

    Notes
    -----
    - Parameter names in the catalog must follow the naming convention of the
      profile's ``param_names`` (case-insensitive). Missing parameters generate
      a warning.
    - The function assumes ``catalog_object`` has exactly one row.
    - This is intended to simplify constructing Lenstronomy profiles dynamically
      from catalogs in simulations or modeling pipelines.
    """

    # Get the required parameter names from the profile
    varnames = getattr(profile, "param_names", [])
    varnames = [v.lower() for v in varnames]

    func_args: {}

    for var in varnames:
        if var in catalog_object.columns:
            func_args[var] = catalog_object[var].values[0]
        else:
            warnings.warn(f"Parameter '{var}' required by {profile.__name__} "
                          f"is missing in catalog_object.", UserWarning)
